    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2">
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
            {isAdmin ? 'Admin - Toàn quyền' : isSeller ? 'Seller - Quyền cơ bản' : 'Không có quyền'}
          </span>
        </div>
      </div>
      ...
      {voucherTypes.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-muted-foreground">Bạn không có quyền tạo voucher.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {voucherTypes.map((voucherType) => {
            ...
            return (
              <Card key={voucherType.id} className={`overflow-hidden rounded-xs ${isAdminVoucher ? 'border-red-200 bg-gradient-to-br from-red-50 to-red-100' : ''}`}>
                <CardContent className="p-0">
                  <div className="flex flex-col h-full">
                    <div className="p-4 flex items-start gap-3">
                      <div className={`h-10 w-10 flex items-center justify-center rounded-full ${isAdminVoucher ? 'bg-red-200' : 'bg-primary/10'}`}>
