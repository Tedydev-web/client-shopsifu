...
interface LocalCartItem extends ApiCartItem {
  selected: boolean; // Chỉ dùng ở UI, không gửi lên server
}
...
export function CartDropdown() {
  ...
  // Cập nhật giỏ hàng khi mở dropdown hoặc khi lastUpdated thay đổi
  useEffect(() => {
    if (isOpen) {
      fetchCart();
    }
    ...
    shopCarts.forEach((shopCart: ShopCart) => {
      shopCart.cartItems.forEach((item: ApiCartItem) => {
        if (selectedItems[item.id]) {
          // Sử dụng virtualPrice từ sản phẩm thay vì price từ SKU
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="divide-y divide-gray-200">
              {allCartItems.map((item: ApiCartItem & { shopName: string }) => {
                ...
                // Lấy hình ảnh chính của sản phẩm thay vì ảnh SKU
