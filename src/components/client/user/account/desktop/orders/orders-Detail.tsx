...
//                 onClick={() => {
//                   if (selectedItem?.productSlug) {
//                     router.push(`/product/${selectedItem.productSlug}`);
//                   }
//                 }}
//               >
//                 Mua lại
...
export default function OrderDetail({ orderId }: OrderDetailProps) {
  ...
  const totalQuantity =
    selectedItem?.quantity ?? order.items?.reduce((sum, item) => sum + item.quantity, 0) ?? 0;
    ...
    <div className="mx-auto bg-[#f5f5f5] space-y-4 text-sm rounded-md">
      ...
      <section className="bg-white rounded-lg border p-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          <span className="text-muted-foreground">•</span>
          <Badge variant={currentStatus.variant || "default"} className="text-xs">
            {currentStatus.label}
          </Badge>
        </div>
